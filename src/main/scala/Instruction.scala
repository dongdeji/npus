...
object NpuCmd {
  val NP_CMD = 2
  def NP_X    = BitPat("b??")
  def NP_LDW  = BitPat("b00");
  def NP_SWAP = BitPat("b00");
  def NP_STK  = BitPat("b00");
}
...
// | funct4:0010 | func3:001 | windoffset[9:0]        | U+size:110 | rd            | coustom0: 0001011 | -- swap result wind unsigned word
// | funct4:0010 | func3:010 |func5:00000| rs1        | U+size:100 | reserve:00000 | coustom0: 0001011 | -- store unsigned byte key to buff
// | funct4:0010 | func3:010 |func5:00000| rs1        | U+size:101 | reserve:00000 | coustom0: 0001011 | -- store unsigned halftword key to buff
// | funct4:0010 | func3:010 |func5:00000| rs1        | U+size:110 | reserve:00000 | coustom0: 0001011 | -- store unsigned word key to buff
// | funct4:0010 | func3:010 |func5:00000| rs1        | U+size:111 | reserve:00000 | coustom0: 0001011 | -- store unsigned doubleword key to buff


/* Automatically generated by parse-opcodes */
object NpInstructions {
  def LPKTWJAL          = BitPat("b0000000??????????????????0001011")
  def LRESWJAL          = BitPat("b0001100??????????????????0001011")
  def LRDJAL            = BitPat("b0001011??????????????????0001011")
  ...
  def STOREKEYBU        = BitPat("b001001000000?????100000000001011")
  def STOREKEYHU        = BitPat("b001001000000?????101000000001011")
  def STOREKEYWU        = BitPat("b001001000000?????110000000001011")
  def STOREKEYDU        = BitPat("b001001000000?????111000000001011")
...
class NpDecode extends DecodeConstants
{
  ...
  //JALR->              List(Y,N,N,Y,N,Y,  A2_IMM, A1_RS1, IMM_I, DW_XPR,FN_ADD, N,M_X,   Y,XD_LLL, CSR.N,Y,NP_LDW ),
    ...
    LRESWJAL  ->        List(Y,N,Y,N,N,N,  A2_SIZE, A1_PC, IMM_UJ,DW_XPR,FN_ADD, N,M_X  , Y,XD_LLL, CSR.N,Y,NP_LDW ),
    LRDJAL    ->        List(Y,N,Y,N,N,N,  A2_SIZE, A1_PC, IMM_UJ,DW_XPR,FN_ADD, N,M_X  , Y,XD_LLL, CSR.N,Y,NP_LDW ),
    LPKTWJALR ->        List(Y,N,N,Y,N,Y,  A2_IMM, A1_RS1, IMM_I, DW_XPR,FN_ADD, N,M_X  , Y,XD_LLL, CSR.N,Y,NP_LDW ),
    LRESWJALR ->        List(Y,N,N,Y,N,Y,  A2_IMM, A1_RS1, IMM_I, DW_XPR,FN_ADD, N,M_X  , Y,XD_LLL, CSR.N,Y,NP_LDW ),
    LRDJALR   ->        List(Y,N,N,Y,N,Y,  A2_IMM, A1_RS1, IMM_I, DW_XPR,FN_ADD, N,M_X  , Y,XD_LLL, CSR.N,Y,NP_LDW ),
  ...
  //ADDIW->             List(Y,N,N,N,N,Y,  A2_IMM, A1_RS1, IMM_I, DW_32,FN_ADD,  N,M_X,   Y,XD_X  , CSR.N,N,NP_STK ),
    STOREKEYBU->        List(Y,N,N,N,N,Y,  A2_X,   A1_RS1, IMM_X, DW_X,  FN_X,   N,M_X,   N,XD_X  , CSR.N,Y,NP_STK ),
    STOREKEYHU->        List(Y,N,N,N,N,Y,  A2_X,   A1_RS1, IMM_X, DW_X,  FN_X,   N,M_X,   N,XD_X  , CSR.N,Y,NP_STK ),
    STOREKEYWU->        List(Y,N,N,N,N,Y,  A2_X,   A1_RS1, IMM_X, DW_X,  FN_X,   N,M_X,   N,XD_X  , CSR.N,Y,NP_STK ),
    STOREKEYDU->        List(Y,N,N,N,N,Y,  A2_X,   A1_RS1, IMM_X, DW_X,  FN_X,   N,M_X,   N,XD_X  , CSR.N,Y,NP_STK ))
}
